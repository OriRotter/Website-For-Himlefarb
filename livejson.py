...
def _initfile(path, data="dict"):
    ...
    if not os.path.exists(path):  # The file doesn't exist
        ...
        with open(path, "w",encoding='utf-8') as f:
            json.dump(data.encode('utf-8'), f,ensure_ascii=False)
        return True
...
class _BaseFile(_ObjectBase):
        ...
        if self.is_caching:
            ...
            with open(self.path, "w",encoding='utf-8') as f:
                try:
                    ...
                    json.dump(data, f, sort_keys=self.sort_keys, indent=indent,ensure_ascii=False)
    ...
    def __exit__(self, *args):
        ...
        with open(self.path, "w",encoding='utf-8') as f:
            # Write the file. Keep user settings about indentation, etc
            indent = self.indent if self.pretty else None
            json.dump(self.cache, f, sort_keys=self.sort_keys, indent=indent,ensure_ascii=False)
        del self.cache
